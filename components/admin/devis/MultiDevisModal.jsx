...
export default function MultiDevisModal({
  ...
  const availableToAdd = useMemo(() => {
    ...
    return base.filter((d) => {
      ...
      return (
        numero.includes(needle) || date.includes(needle) || type.includes(needle)
      );
    });
  }, [pool, lines, pickerQ]);
  ...
  const canSubmit =
    lines.length > 0 && lines.every((l) => l.articleId && l.qty > 0);
        ...
        <div className="sticky top-0 z-10 rounded-t-2xl border-b bg-white/95 backdrop-blur px-6 py-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3
                id="multi-devis-title"
                className="text-lg sm:text-xl font-semibold text-[#0B1E3A]"
              >
                Créer un devis <span className="text-slate-500">(multi-demandes)</span>
              </h3>
              {client && (
                <div className="mt-1 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-700">
                  <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#F7C600] text-[#0B1E3A] font-semibold">
                    {String(client?.prenom || client?.nom || "?")
                      .charAt(0)
                      .toUpperCase()}
                  </span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          <div className="mb-4 flex justify-between items-center">
            <h4 className="text-sm font-semibold text-slate-700">
              Demandes sélectionnées ({lines.length})
            </h4>
            <div className="relative">
              <button
                ...
                /* ✅ Vert clair */
                className="inline-flex items-center gap-2 rounded-xl rounded-xl border border-emerald-200 bg-emerald-50  px-4 py-2 font-semibold text-emerald-700 hover:bg-emerald-100 disabled:opacity-50"
