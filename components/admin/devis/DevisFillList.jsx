...
export default function DevisFilList() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (res.status === 401) {
        router.push(
          `/fr/login?next=${encodeURIComponent("/fr/admin/devis/fil")}`
        );
        return;
      }
      if (res.status === 403) {
    ...
    (async () => {
      const pairs = await Promise.all(
        items.map(async (d) => {
          try {
            const r = await fetch(
              `${BACKEND}/api/devis/admin/by-demande/${
                d._id
  ...
  async function viewDocByIndex(id, index) {
    try {
      const res = await fetch(
        `${BACKEND}/api/admin/devis/fil/${id}/document/${index}`,
        {
          method: "GET",
          credentials: "include",
        }
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      <DevisModal
        ...
        articleKinds={["fil", "fil"]} // 🔥 filtre uniquement les articles Fil
      />
